...
async def ensure_private_setup_message(*, force_new: bool = False) -> bool:
    """Создаёт/обновляет сообщение с кнопкой формы в приватке.

    force_new=True — принудительно отправить новое сообщение и сохранить его ID.
    Это полезно, если старое сообщение потерялось/утонуло/его удалили вручную.
    """
    try:
        ...
        if stored_id:
            try:
                ...
                if client.user and old_msg.author and old_msg.author.id == client.user.id:
                    # Сообщение уже есть — не молчим, а обновляем текст/кнопку на актуальные.
                    await old_msg.edit(
                        content=PRIVATE_SETUP_MESSAGE,
