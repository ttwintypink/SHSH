...
from admin_text_commands import handle_admin_text_command
from event_manager import setup_event_manager
from channel_protection import (
    ensure_enabled_snapshots,
    handle_guild_channel_update as protect_handle_guild_channel_update,
    handle_guild_channel_create as protect_handle_guild_channel_create,
    handle_guild_channel_delete as protect_handle_guild_channel_delete,
)
...
async def on_ready():
    ...
    setup_event_manager()
    await ensure_enabled_snapshots(client)
...
async def on_guild_channel_create(channel):
    try:
        await protect_handle_guild_channel_create(channel)
    except Exception as e:
        print(f"[SH] WARNING: channel-protect create handler failed: {type(e).__name__}: {e}")
...
async def on_guild_channel_update(before, after):
    try:
        await protect_handle_guild_channel_update(before, after)
    except Exception as e:
        print(f"[SH] WARNING: channel-protect update handler failed: {type(e).__name__}: {e}")


@client.event
async def on_guild_channel_delete(channel):
    try:
        await protect_handle_guild_channel_delete(channel)
    except Exception as e:
        print(f"[SH] WARNING: channel-protect delete handler failed: {type(e).__name__}: {e}")
...
async def on_message(message: discord.Message):
    ...
    # ------------------------------------------------------
    # Команды .help/.block/.unblock.
    # Обрабатываем до заявки-логики, чтобы команды не меняли статус канала.
    # ------------------------------------------------------
    try:
        if await handle_admin_text_command(client, message):
            return
    except Exception as e:
        print(f"[SH] WARNING: admin text command failed: {type(e).__name__}: {e}")
        return
        ...
        if opener:
            if isinstance(message.author, discord.Member) and not message.author.bot:
                if message.author.id == opener.id:
                    await update_ticket_channel_status(
                        message.channel,
                        opener,
                        STATUS_USER_WAITING,
