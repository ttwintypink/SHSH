...
async def _send_result_log(
    ...
    target_text = f"<@{target_id}>" if target_id else "указанному пользователю"

    if ok:
        text = (
            f"✅ | Супер! Уведомление от модератора <@{CALL_COMMAND_ALLOWED_USER_ID}> "
            f"пользователю {target_text} было успешно отправлено через команду .call . "
            "Пользователь получил уведомление в личные сообщения и должен в скором времени выйти на связь."
        )
    else:
        text = (
            f"❌ | Отказано! Уведомление от модератора <@{CALL_COMMAND_ALLOWED_USER_ID}> "
            f"пользователю {target_text} не отправлено по неизвестной причине."
        )

    log_channel = await _get_log_channel(client, message)

    # Если канал логов почему-то недоступен, хотя бы напишем результат туда, где была команда.
