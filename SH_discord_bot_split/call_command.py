...
async def _send_channel_confirmation(
    message: discord.Message,
    ok: bool,
    target_id: int | None,
) -> None:
    """Отправляет короткий результат в тот канал, где использовали .call."""
    if not isinstance(message.channel, discord.TextChannel):
        return

    user_text = f"<@{target_id}>" if target_id else "пользователю"
    if ok:
        text = f"✅ Уведомление пользователю {user_text} отправлено. Подробности в логах."
    else:
        text = f"❌ Уведомление пользователю {user_text} не отправлено. Подробности в логах."
...
async def handle_call_command(client: discord.Client, message: discord.Message) -> bool:
    ...
    if not message.guild or not isinstance(message.channel, discord.TextChannel):
        ...
        await _send_channel_confirmation(message, False, target_id)
        return True

    if target_id is None:
        await _send_result_log(client, message, False, None)
        await _send_channel_confirmation(message, False, None)
        return True
    ...
    await _send_channel_confirmation(message, ok, target_id)
    return True
