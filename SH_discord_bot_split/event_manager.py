...
def _get_active_events() -> list[EventRecord]:
    with _connect() as con:
        rows = con.execute(
            "SELECT message_id, guild_id, channel_id, creator_id, title, description, start_ts, end_ts, member_limit, status, cleaned_at "
            "FROM clan_events WHERE status IN ('scheduled', 'active') ORDER BY start_ts ASC;"
        ).fetchall()
...
def _has_active_event(guild_id: int) -> EventRecord | None:
    ...
    with _connect() as con:
        row = con.execute(
            "SELECT message_id, guild_id, channel_id, creator_id, title, description, start_ts, end_ts, member_limit, status, cleaned_at "
            "FROM clan_events WHERE guild_id=? AND status IN ('scheduled', 'active') AND end_ts>? ORDER BY start_ts ASC LIMIT 1;",
            (guild_id, now),
        ).fetchone()
...
def _cancel_guild_events(guild_id: int) -> int:
    with _connect() as con:
        cur = con.execute(
            "UPDATE clan_events SET status='deleted', cleaned_at=? WHERE guild_id=? AND status IN ('scheduled', 'active');",
...
class EventCreateModal(discord.ui.Modal, title="Создание события"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if start_dt.timestamp() < time.time() - 90:
        ...
        # Если время публикации уже наступило за время выдачи ролей — публикуем сразу.
        if int(time.time()) >= rec.start_ts:
            try:
                await publish_scheduled_event(rec)
                await interaction.followup.send(
                    f"✅ Роли выданы и проверены у людей: {eligible}. Событие опубликовано.",
                    ephemeral=True,
                )
                return
            except Exception as e:
                ...
                try:
                    ...
                    pass
                await interaction.followup.send(
                    f"❌ Роли выдал, но не смог опубликовать событие. Роли очищены. Ошибка: `{type(e).__name__}: {e}`",
                    ephemeral=True,
                )
                return

        await interaction.followup.send(
            f"✅ Роль 'ещё не отметился' выдана и проверена у людей: {eligible}. Событие будет опубликовано <t:{rec.start_ts}:R>.",
            ephemeral=True,
        )
...
async def event_clear_roles(interaction: discord.Interaction):
    ...
    try:
        changed, failed = await cleanup_event_roles(interaction.guild)
        cancelled = _cancel_guild_events(interaction.guild.id)
    except Exception as e:
        await interaction.followup.send(f"❌ Не смог очистить роли: `{type(e).__name__}: {e}`", ephemeral=True)
        return
    await interaction.followup.send(
        f"✅ Временные роли сняты у {changed} пользователей. Ошибок: {failed}. Остановлено событий: {cancelled}.",
        ephemeral=True,
    )
...
async def event_list(interaction: discord.Interaction):
    ...
    def _line(e: EventRecord) -> str:
        state = "запланировано" if e.status == "scheduled" else "опубликовано"
