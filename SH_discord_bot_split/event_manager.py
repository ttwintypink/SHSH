...
# Напоминания. Каждое сообщение удаляется через час.
REMINDER_IMMEDIATE_OFFSET_SECONDS = 0
...
PREPARE_HARD_TIMEOUT_SECONDS = 240
...
_PREPARING_STARTED_AT: dict[int, float] = {}
_ROLE_JOB_TASKS: dict[int, asyncio.Task] = {}


def _spawn_task(coro, *, name: str | None = None) -> asyncio.Task:
    try:
    ...
    def _done(t: asyncio.Task) -> None:
        try:
            ...
            _log(f"[ClanEvent] task cancelled name={name}")
            return
        except Exception as e:
            _log(f"[ClanEvent] task result read failed name={name}: {type(e).__name__}: {e}")
            return
        if exc is not None:
            _log(f"[ClanEvent] task crashed name={name}: {type(exc).__name__}: {exc}")
...
def _clock_emoji(guild: discord.Guild) -> str:
    # Пользовательский emoji на сервере. Если Discord отдаёт другое имя, есть fallback.
    for name in ("clock~1", "clock_1", "clock"):
...
def build_event_embed(guild: discord.Guild, rec: EventRecord) -> discord.Embed:
    ...
    if guild.icon:
        try:
            embed.set_author(name=f"{guild.name} • Event Manager", icon_url=guild.icon.url)
        except Exception:
            embed.set_author(name=f"{guild.name} • Event Manager")
    else:
        embed.set_author(name=f"{guild.name} • Event Manager")

    embed.add_field(
        name=f"{clock_icon} Время",
        value=(
            f"**Публикация:** <t:{rec.start_ts}:F>\n"
            f"**Закрытие отметок:** <t:{rec.end_ts}:t> МСК\n"
            f"**Статус:** {clock_icon} {relative}"
        ),
        inline=False,
    )
...
async def _prepare_event_core(message_id: int) -> None:
    _log(f"[ClanEvent] prepare core enter id={message_id}")
    rec = _get_event(message_id)
    if rec is None:
        _log(f"[ClanEvent] prepare core stop id={message_id}: event not found in DB")
        return
    if rec.status != "preparing":
        _log(f"[ClanEvent] prepare core stop id={message_id}: status={rec.status}")
        return
...
async def _prepare_event_after_create(message_id: int) -> None:
    """Prepare roles, then schedule or publish event."""
    _log(f"[ClanEvent] prepare task enter id={message_id}")
    if message_id in _PREPARING_EVENTS:
        _log(f"[ClanEvent] prepare task skip id={message_id}: already preparing")
        return
    _PREPARING_EVENTS.add(message_id)
    _PREPARING_STARTED_AT[message_id] = time.time()
    try:
        ...
        _PREPARING_STARTED_AT.pop(message_id, None)
        _ROLE_JOB_TASKS.pop(message_id, None)
        _log(f"[ClanEvent] prepare task exit id={message_id}")

def _schedule_prepare_job(message_id: int) -> None:
    task = _ROLE_JOB_TASKS.get(message_id)
    if task is not None and not task.done():
        _log(f"[ClanEvent] prepare job already running id={message_id}")
        return
    _log(f"[ClanEvent] scheduling prepare job id={message_id}")
    _ROLE_JOB_TASKS[message_id] = _spawn_task(_prepare_event_after_create(message_id), name=f"clan-event-prepare-{message_id}")


def _schedule_publish_job(message_id: int) -> None:
    _log(f"[ClanEvent] scheduling publish job id={message_id}")
    _spawn_task(_publish_when_due(message_id), name=f"clan-event-publish-{message_id}")
...
async def _send_event_reminder(rec: EventRecord, offset_seconds: int, channel: discord.TextChannel | None = None) -> bool:
    """Отправляет одно конкретное напоминание и удаляет его через час."""
    if rec.status != "active":
        return False
    if _reminder_was_sent(rec.message_id, offset_seconds):
        return False
    ...
    now = _now_ts()
    if now >= rec.end_ts:
        _mark_reminder_sent(rec.message_id, offset_seconds, None)
        return False

    if offset_seconds == REMINDER_IMMEDIATE_OFFSET_SECONDS:
        content = f"<@&{EVENT_NOT_VOTED_ROLE_ID}> , было опубликовано новое событие. Мы ждем вашей отметки!"
    elif offset_seconds == 24 * 3600:
        content = f"<@&{EVENT_NOT_VOTED_ROLE_ID}> , событие закончится через 24 часа. Мы ждем вашей отметки!"
    elif offset_seconds == 12 * 3600:
        content = f"<@&{EVENT_NOT_VOTED_ROLE_ID}> , событие закончится через 12 часов. Мы ждем вашей отметки!"
    elif offset_seconds == 6 * 3600:
        content = f"<@&{EVENT_NOT_VOTED_ROLE_ID}> , событие закончится через 6 часов. Мы ждем вашей отметки!"
    elif offset_seconds == 3 * 3600:
        content = f"<@&{EVENT_NOT_VOTED_ROLE_ID}> , событие закончится через 3 часа. Мы ждем вашей отметки!"
    elif offset_seconds == 1 * 3600:
        # Пингуем все временные роли, кроме "ещё не отметился".
        # То есть: accepted / declined / tentative.
        active_temp_roles = (
            EVENT_ACCEPTED_ROLE_ID,
            EVENT_DECLINED_ROLE_ID,
            EVENT_TENTATIVE_ROLE_ID,
        )
        role_mentions = " ".join(f"<@&{role_id}>" for role_id in active_temp_roles)
        content = (
            f"{role_mentions}\n\n"
            "Внимание, до конца события остался 1 час. "
            "Будьте на связи, скоро будет движ!"
        )
    else:
        content = f"<@&{EVENT_NOT_VOTED_ROLE_ID}> , событие скоро закончится. Мы ждем вашей отметки!"
    ...
    try:
        ...
        _mark_reminder_sent(rec.message_id, offset_seconds, msg.id)
        _spawn_task(_delete_later(msg, REMINDER_DELETE_AFTER_SECONDS), name="clan-event-delete-reminder")
        _log(f"[ClanEvent] reminder sent event={rec.message_id} offset={offset_seconds} channel={channel.id} msg={msg.id}")
        return True
    except Exception as e:
        _log(f"[ClanEvent] reminder failed event={rec.message_id} offset={offset_seconds}: {type(e).__name__}: {e}")
        return False
...
async def publish_scheduled_event(rec: EventRecord) -> None:
    ...
    try:
        ...
        await _send_event_reminder(rec, REMINDER_IMMEDIATE_OFFSET_SECONDS, channel=channel)
    finally:
...
async def _process_scheduled_publish(rec: EventRecord) -> None:
    if rec.status == "preparing":
        task = _ROLE_JOB_TASKS.get(rec.message_id)
        if task is None or task.done() or rec.message_id not in _PREPARING_EVENTS:
            _log(f"[ClanEvent] worker sees preparing event; ensure prepare job id={rec.message_id} task_done={None if task is None else task.done()}")
            _schedule_prepare_job(rec.message_id)
        else:
            started = _PREPARING_STARTED_AT.get(rec.message_id, time.time())
            age = time.time() - started
            if age > PREPARE_HARD_TIMEOUT_SECONDS + 30:
                _log(f"[ClanEvent] preparing watchdog timeout id={rec.message_id} age={age:.1f}; cancelling task")
                task.cancel()
                _set_event_status(rec.message_id, "role_error")
        return
    if rec.status != "scheduled":
        return
    if _now_ts() < rec.start_ts:
...
async def _process_event_reminders(rec: EventRecord) -> None:
    ...
    # Напоминания считаются от времени закрытия отметок.
    # Если событие короткое, дальние напоминания не отправляются.
    for offset in REMINDER_OFFSETS_SECONDS:
        if _reminder_was_sent(rec.message_id, offset):
            continue

        remind_at = rec.end_ts - offset

        # Не отправляем "через 24/12/6/3/1 час", если этот момент был ещё до публикации события.
        if remind_at < rec.start_ts:
            _mark_reminder_sent(rec.message_id, offset, None)
            continue

        if now < remind_at:
            continue

        # Если бот был выключен и сильно опоздал, не шлём неправильное старое напоминание.
        if now - remind_at > REMINDER_GRACE_SECONDS:
            _mark_reminder_sent(rec.message_id, offset, None)
            _log(f"[ClanEvent] reminder skipped as stale event={rec.message_id} offset={offset}")
            continue

        await _send_event_reminder(rec, offset)
...
def setup_event_manager() -> None:
    ...
    if not getattr(client, "_clan_event_worker_started", False):
        ...
        _spawn_task(event_background_worker(), name="clan-event-worker")
