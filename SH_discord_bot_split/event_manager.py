...
async def assign_not_voted_to_humans(guild: discord.Guild) -> tuple[int, int]:
    ...
    try:
        ...
        _log(f"[ClanEvent] guild chunk warning guild={guild.id}: {type(e).__name__}: {e}")

    members = [m for m in guild.members if not m.bot and m.id not in EVENT_ROLE_SKIP_USER_IDS]
    _log(f"[ClanEvent] assign_not_voted start guild={guild.id} members={len(members)}")

    for idx, member in enumerate(members, start=1):
        try:
            remove = [role for key, role in roles.items() if role in member.roles and role != not_voted]
            if remove:
    ...
    _log(f"[ClanEvent] assign_not_voted done guild={guild.id} changed={changed} failed={failed} members={len(members)}")
...
async def cleanup_event_roles(guild: discord.Guild) -> tuple[int, int]:
    """Снимает event-роли быстрее и надёжнее."""
    roles = _get_event_roles(guild)
    if roles is None:
        raise RuntimeError("Одна или несколько event-ролей не найдены на сервере.")
