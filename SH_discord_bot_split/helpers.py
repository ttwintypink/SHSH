...
def message_contains_trigger(msg: discord.Message) -> bool:
    ...
    # Поддерживаем оба варианта текста, чтобы бот работал и со старым сообщением Ticket Tool
    # про "тикет", и с новым/переименованным вариантом про "заявку".
    trigger_variants = {
        trigger_norm,
        _normalize_text("вы серьезно хотите закрыть данный тикет"),
        _normalize_text("вы серьезно хотите закрыть данную заявку"),
    }

    return any(v and v in joined_norm for v in trigger_variants)
