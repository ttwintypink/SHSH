...
async def move_application_channel_to_top(channel: discord.TextChannel) -> bool:
    ...
    try:
        ...
        # Если канал уже стоит сразу под "логи-причин", не дёргаем Discord API лишний раз.
        if log_channel is not None:
            for index, ch in enumerate(text_channels):
                if ch.id == log_channel.id:
                    if index + 1 < len(text_channels) and text_channels[index + 1].id == channel.id:
                        return False
                    break
        elif text_channels and text_channels[0].id == channel.id:
            return False
    except Exception:
        pass
