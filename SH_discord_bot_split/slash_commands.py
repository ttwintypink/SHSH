...
from member_cache import safe_fetch_member, warm_guild_member_cache


ROLE_SYNC_DELAY_SECONDS = 0.25
...
async def _get_guild(client: discord.Client, guild_id: int) -> discord.Guild | None:
    guild = client.get_guild(guild_id)
    if guild is not None:
        return guild
    try:
        fetched = await client.fetch_guild(guild_id)
        return fetched
    except Exception:
        return None
...
async def _sync_member_roles(
    ...
    member_id: int,
    *,
    allow_private_fetch: bool = True,
) -> tuple[bool, str]:
    public_guild = await _get_guild(client, PUBLIC_GUILD_ID)
    private_guild = await _get_guild(client, PRIVATE_GUILD_ID)
    ...
    public_member = await safe_fetch_member(public_guild, member_id, allow_fetch=True)
    if public_member is None:
        return False, f"Пользователь {member_id} не найден в паблике."

    # В /syncall не делаем сотни REST fetch_member по приватке.
    # Сначала прогреваем кэш через Gateway chunk, затем смотрим get_member/cache.
    private_member = await safe_fetch_member(private_guild, member_id, allow_fetch=allow_private_fetch)
...
@tree.command(name='sync', description='Синхронизировать NULL/FUN NULL для одного участника')
...
async def sync_cmd(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ok, text = await _sync_member_roles(interaction.client, user.id, allow_private_fetch=True)
...
@tree.command(name='syncall', description='Синхронизировать NULL/FUN NULL для всех участников паблика')
async def syncall_cmd(interaction: discord.Interaction):
    ...
    public_guild = await _get_guild(interaction.client, PUBLIC_GUILD_ID)
    private_guild = await _get_guild(interaction.client, PRIVATE_GUILD_ID)
    if public_guild is None:
        await interaction.followup.send(f'❌ Публичный сервер {PUBLIC_GUILD_ID} не найден.', ephemeral=True)
        return
    if private_guild is None:
        await interaction.followup.send(f'❌ Приватный сервер {PRIVATE_GUILD_ID} не найден.', ephemeral=True)
        return

    # Главное исправление 429: вместо сотен GET /members прогреваем кэш приватки одним Gateway chunk.
    await warm_guild_member_cache(private_guild)
    ...
    for member in public_guild.members:
        ...
        processed += 1
        try:
            ok, text = await _sync_member_roles(interaction.client, member.id, allow_private_fetch=False)
        ...
        # Мягкая пауза, чтобы Discord не душил массовую выдачу/снятие ролей.
        if processed % 4 == 0:
            await asyncio.sleep(ROLE_SYNC_DELAY_SECONDS)
