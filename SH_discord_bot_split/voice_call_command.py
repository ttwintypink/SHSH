...
async def _send_voice_call_report(
    ...
    details = (
        "Пользователь был вызван на проверку. Канал: **ожидание проверки**."
        if ok
...
async def handle_voice_call_command(client: discord.Client, message: discord.Message) -> bool:
    ...
    # В тикет отправляем нормальную кликабельную ссылку,
    # а preview-карточку Discord ниже подавляем через suppress_embeds.
    text = (
